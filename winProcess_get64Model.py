...
if __name__ == '__main__':
    hwnd = FindWindow(None, u"微信")  # 获取窗口句柄
    print(hwnd)
    m = Memory64(hwnd)  # 我们创建一个内存的操作类

    ModuleBaseAddr = m.GetBaseAddr("WeChatWin.dll")
    ...
    """
    addr = m.ReadVirtualMemory64(ModuleBaseAddr + 0x166E164)  # addr = ["GameAssembly.dll"+0166E164]
    addr = m.ReadVirtualMemory64(addr + 0x5C)  # addr = [addr + 0x5C]
    addr = m.ReadVirtualMemory64(addr)  # addr = [addr]
    addr = m.ReadVirtualMemory64(addr + 0x68)  # addr = [addr + 0x68]
    addr = m.ReadVirtualMemory64(addr + 0x70)  # addr = [addr + 0x70]
    addr = m.ReadVirtualMemory64(addr + 0x70)  # addr = [addr + 0x70]
    addr = m.ReadVirtualMemory64(addr + 0xC)  # addr = [addr + 0xC]
    print(addr)  # 这个时候我们的addr其实已经是存放x坐标的地址了，我们可以在下面进行测试

    ret = m.ReadVirtualMemory64(addr)  # 这里记得只读取四个字节，多了的话读取到的数据肯定就是错误的了
    print("x坐标", ret)
